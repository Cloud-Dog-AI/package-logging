...
class AuditMiddleware:
    ...
    async def __call__(
        ...
        if server:
            ...
            server_address = f"{server_host}:{server_port}" if server_port is not None else str(server_host)
        source_address = f"{client_ip}:{client_port}" if client_port is not None else client_ip
        ...
        *,
        ...
        try:
            ...
            event = audit._build_event(
                ...
                duration_ms=duration_ms,
                component="http",
                source_application=_header_value(scope, "user-agent") or "http-client",
                source_address=source_address,
