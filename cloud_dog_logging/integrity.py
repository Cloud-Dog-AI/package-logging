...
class AuditIntegrityVerifier:
    ...
    def _bind_logging_context(self) -> None:
        """Ensure verifier-emitted app logs carry stable service context."""
    ...
    def _write_integrity_record(self, record: dict[str, Any]) -> None:
        ...
        try:
            self._integrity_log_path.parent.chmod(0o700)
        except OSError:
            pass
        with self._integrity_log_path.open("a", encoding="utf-8") as stream:
            stream.write(json.dumps(record, ensure_ascii=False) + "\n")
        try:
            self._integrity_log_path.chmod(0o600)
        except OSError:
            pass
