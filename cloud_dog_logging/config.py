...
class LogConfig:
    ...
    # Declarative handler-type selection. ``None`` (default) preserves the
    # legacy behaviour where handler choice is driven by ``app_log_file`` +
    # ``console_output``. Any of the strings ``file``/``console``/``dual``/
    # ``rotating`` (case-insensitive) or :class:`HandlerType` members may be
    # supplied. See ``cloud_dog_logging.handler_types.HandlerType``.
    handlers: list[str] | None = None
    # Top-level JSON fields to emit (in addition to the default schema)
    # using attribute names supplied by ``register_log_field_provider``.
    ...
    def from_dict(cls, config: dict[str, Any]) -> LogConfig:
        ...
        handlers = log_section.get("handlers")
        if isinstance(handlers, str):
            handlers = [handlers]
        elif handlers is not None and not isinstance(handlers, list):
            handlers = None
        # Coerce HandlerType members (or anything stringable) into plain
        # strings so the resulting LogConfig stays JSON/dict-friendly.
        if isinstance(handlers, list):
            normalised: list[str] = []
            for item in handlers:
                if hasattr(item, "value"):
                    normalised.append(str(item.value))  # type: ignore[arg-type]
                else:
                    normalised.append(str(item))
            handlers = normalised

        extra_fields = log_section.get("extra_fields")
        if isinstance(extra_fields, str):
            extra_fields = [extra_fields]
        elif extra_fields is not None and not isinstance(extra_fields, list):
            extra_fields = None
