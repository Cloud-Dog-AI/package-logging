...
class TestActor:
    ...
    def test_to_dict(self) -> None:
        ...
        d = actor.to_dict()
        assert d == {"type": "user", "id": "u-1", "roles": ["admin"], "ip": "unknown"}

    def test_to_dict_no_roles(self) -> None:
        actor = Actor(type="user", id="u-1")
        d = actor.to_dict()
        assert d == {"type": "user", "id": "u-1", "roles": [], "ip": "unknown"}
...
class TestAuditEvent:
    ...
    def test_required_fields_enforced(self) -> None:
        ...
        assert event.source_process
        assert event.source_application == "test-service"
    ...
    def test_timestamp_explicit(self) -> None:
        event = self._make_event(timestamp="2026-01-01T00:00:00.000Z")
        assert event.timestamp == "2026-01-01T00:00:00.000Z"

    def test_timestamp_requires_utc_milliseconds(self) -> None:
        with pytest.raises(ValueError, match="timestamp must be UTC ISO 8601 with milliseconds"):
            self._make_event(timestamp="2026-01-01T00:00:00.000000Z")
    ...
    def test_to_dict_all_fields(self) -> None:
        ...
        assert d["component"] == "auth-api"
        assert d["source_host"] == "host-1"
        assert d["source_process"] == "pid-123"
        assert d["source_application"] == "pytest"
        assert d["source_address"] == "10.0.0.1"
        assert d["destination_address"] == "10.0.0.2"
        assert d["affected_files"] == ["/workspace/rules.md"]
